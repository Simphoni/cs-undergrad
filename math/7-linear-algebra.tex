...
\tinysec{定义}$\mathcal{R}(A^T)$ 为 $A$ 的\concept{行（向量）空间}；$\mathrm{rank}(A)=\mathrm{rank}(A^T)$；$\mathrm{rank}(A)=n$ 定义为\concept{列满秩}，同理有\concept{行满秩}。\\
...
\tinysec{定义}$\bm{b}$ 向 $\bm{a}$ 的\concept{垂直投影} 为 $\hat{x}=\frac{\bm{a}^T\bm{b}}{\bm{a}^T\bm{a}}$；向量 $\frac{\bm{a}^T\bm{b}}{\bm{a}^T\bm{a}}\bm{a}$ 称为向量 $\bm{b}$ 向直线 $\mathrm{span}(\bm{a})$ 的投影。\\

\tinysec{定理}\concept{（Cauchy-Schwarz 不等式）} $|\bm{a}^T\bm{b}|\le \|a\|\|b\|$。\\
...
\tinysec{定义}若方阵 $Q$ 满足 $Q^TQ=I_n$，则称 $Q$ 为 $n$ 阶\concept{正交矩阵}；$Q$ 的行、列向量各自构成 $\linsp{n}$ 的一组标准正交基。\\
