...
module lab3_tb;
  ...
  initial begin
    ...
    for (int i = 1; i < 32; i = i + 1) begin
      ...
      push_btn = 0;
      #3000;
      $display("%b", `inst_poke(rd, imm));
      $display("poked[%d]: %x", i, imm);
    end
    ...
    for (int i = 0; i < 32; i = i + 1) begin
      ...
      push_btn = 0;
      #3000;
      $display("%b", `inst_peek(rd, imm));
      $display("peek[%d]: %x", i, leds);
    end
    // TODO: 随机测试各种指令
    for (int i = 1; i < 32; i = i + 1) begin
      #100;
      rd = 2;   // only lower 5 bits
      rs1 = 4;
      rs2 = $urandom_range(3, 31);
      opcode = $urandom_range(1, 10);
      dip_sw = `inst_rtype(rd, rs1, rs2, ADD);
      push_btn = 1;
      #100;
      push_btn = 0;
      #1000;
