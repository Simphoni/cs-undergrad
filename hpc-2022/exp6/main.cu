...
__global__ void naive(float *dev_input, float *dev_output, int H, int W, int kernel_size)
{
    ...
    dev_output[thread_id_x * W + thread_id_y] = ans;
}
...
double test_func(int block_size_x, int block_size_y, bool use_shared_memory = 0, bool validation = 0)
{
    ...
    t /= test;
    return t;
    //std::cout << mode.substr(0,5) << "\t\t" << block_size_x << ' ' << block_size_y << ' ' << "Exec-time: " << t << " ms" << std::endl;
    //std::cout << mode.substr(0,5) << "\t" << block_size_x * block_size_y << ' ' << "Exec-time: " << t << " ms" << std::endl;
}
...
int main()
{
    ...
    for (int i = 32; i <= 1024; i += 32) // blockDim.x
        for (int j = 1; j <= 32; ++j)    // blockDim.y
        {
            if (i * j > 1024)
                break;
	    double t1 = test_func(i, j, false, 0); // default: no validation
	    double t2 = test_func(i, j, true, 0);
	    printf("%d %d %.5lf\n", i, j, t1 - t2);
	    fflush(stdout);
        }
    return 0;
