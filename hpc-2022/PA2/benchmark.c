...
int parse_args(dist_grid_info_t *info, int *nsteps, int argc, char **argv);
...
int main(int argc, char **argv) {
    int nt, thread_scheme, status, p_id;
    ...
    ABORT_IF_ERROR(parse_args(&info, &nt, argc, argv))

    MPI_Barrier(MPI_COMM_WORLD);
    start = MPI_Wtime();
    create_dist_grid(&info);
    MPI_Barrier(MPI_COMM_WORLD); 
    ...
    if(status == 0) {
        ...
        if(p_id == 0) { 
            double t = end - start; 
            double gflops = 1e-9 * info.global_size_x * info.global_size_y * info.global_size_z * nt * (7 * 2 - 1) / t;

            printf("7-point stencil - %s:\nSize (%d x %d x %d), Timestep %d\n", version_name, info.global_size_x, info.global_size_y, info.global_size_z, nt);
...
void print_help(const char *argv0, int p_id) {
    if(p_id == 0) {
        printf("USAGE: %s 7 <nx> <ny> <nz> <nt> <input-file> [<answer-file>]\n", argv0);
    }
}
