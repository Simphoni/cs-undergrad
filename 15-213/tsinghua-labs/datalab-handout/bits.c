...
int bitReverse(int x) {
  ...
  x >>= 16;
  ret <<= 16;
  ret = ret | ((x & 0xf) << 12) | ((x & 0xf0) << 4) | ((x >> 4) & 0xf0) | ((x >> 12) & 0xf); // 12 op, 16b done
  return ret;
}
/* 
...
int isLessOrEqual(int x, int y) {
  int z = x + (~ y) + 1; // z = x - y
  return ( ( ((z >> 31) & 1) | !(x ^ y) ) & !(((x ^ y) >> 31) & 1) ) // (z < 0 || z == 0) & (sgn(x) == sgn(y))
    | ((!(y >> 31)) & (x >> 31) & 1); // y >= 0 > x
}
/*
